...
export class AiChatbotService {
  ...
  constructor(
    ...
    private aiConversationModel: Model<AiConversation>,
  ...
  async getChatResponse(
    ...
    try {
      ...
      try {
        ...
        const userFields = userMeta.columns
          .map((c) => `${c.propertyName} (${c.type})`)
          .join(', ');

        const companyMeta = this.dataSource.getMetadata('Company');
        const companyFields = companyMeta.columns
          .map((c) => `${c.propertyName} (${c.type})`)
          .join(', ');

        const jobMeta = this.dataSource.getMetadata('Job');
        const jobFields = jobMeta.columns
          .map((c) => `${c.propertyName} (${c.type})`)
          .join(', ');
...
`;
      ...
      // The main system prompt lives in the Python service (sent as 'system' role).
      // NestJS only appends dynamic DB context if available.
      ...
      if (error.response) {
        this.logger.error(
          `AI Service Error (${error.response.status}): ${error.message}`,
        );
      } else {
      ...
      try {
        res
          .status(500)
          .write(
            `data: ${JSON.stringify({ error: 'عذراً، الشات بوت غير متاح حالياً.' })}\n\n`,
          );
        res.end();
      } catch (e) {}
    }
  ...
  private async saveToMongo(
    ...
    try {
      ...
      await this.aiConversationModel
        .updateOne(
          { userId },
